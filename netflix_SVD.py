...
@njit
def _compute_val_metrics(X_test, pu, qi, pu_1, qi_1):
    pu[-1] = pu_1
    qi[-1] = qi_1
    residuals = []
    preds = []
...
#     ]
#
#     @use_named_args(space_SVD_pure)
#     def f_SVD_pure(reg, n_features):
...
f = open(path + dataset + "_pure" + "_params", "rb")
print(evaluate_pure(f)[1])
